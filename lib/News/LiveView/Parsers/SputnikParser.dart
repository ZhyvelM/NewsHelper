...
class SputnikParser extends IParserInterface {
  ...
  Future<List<News>> getNews(DateTime day) async {
    ...
    final _path =
        "/news/${day.year}${day.month}${day.day.toString().length == 1 ? "0" : ""}${day.day}/";
    String path =
        'https://sputnik.by/news/${day.year}${day.month}${day.day.toString().length == 1 ? "0" : ""}${day.day}/';
    ...
    news.forEach((element) {
      ...
      if (element.getElementsByClassName("b-plainlist__title").isNotEmpty &&
          element.getElementsByClassName("b-plainlist__announce").isNotEmpty &&
          element.getElementsByClassName("b-plainlist__date").isNotEmpty) {
        news.title = element
            .getElementsByClassName("b-plainlist__title")[0]
            .getElementsByTagName("a")[0]
            .text;
        news.description = element
            .getElementsByClassName("b-plainlist__announce")[0]
        ...
        news.dateTime = DateTime(
            ...
            day.month,
            day.day,
            int.parse(date.split(" ").first.split(":").first),
            int.parse(date.split(" ").first.split(":").last));
